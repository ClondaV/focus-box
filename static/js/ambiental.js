...
class FireParticle {
    ...
    draw() {
        if (this.alpha <= 0) return;
        ctx.beginPath();
        const hue = 20 + Math.random() * 30; // Range: orange to red
...
class AmbientFireParticle {
    constructor() {
        this.reset(); // ✅ make sure this is defined
    }
    ...
    update() {
        ...
        if (this.alpha <= 0 || this.y < -10) {
            this.reset(); // ✅ this must also be valid
        }
    }
...
function animate() {
    ...
    // ---- Fire State ----
        ...
        // Continuously add ambient fire particles
        for (let i = 0; i < 2; i++) {
